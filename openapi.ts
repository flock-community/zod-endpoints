...
import {ZodLiteral, ZodObject, ZodString, ZodTuple, ZodTypes, ZodUndefined, ZodUnion} from "../zod/src/index.ts";

import {objectUtil} from "../zod/src/helpers/objectUtil.ts";
...
type HttpResponse = {
    ...
    headers: ZodObject<{}, any, Record<PropertyKey, string| number>> | ZodUndefined;
    content: Component | ZodUndefined
};
type HttpResponseObject = ZodObject<HttpResponse, any, objectUtil.Flatten<objectUtil.ObjectType<HttpResponse>>>;
...
export type Http = {
    ...
    path: ZodTuple<[ZodLiteral<any> | ZodString | Parameter, ...(ZodLiteral<any> | ZodString | Parameter)[]]>;
    query: ZodObject<{}, any, Record<PropertyKey, string| number>> | ZodUndefined;
    headers: ZodObject<{}, any, Record<PropertyKey, string | string[]>> | ZodUndefined;
    responses: ZodUnion<[HttpResponseObject, HttpResponseObject, ...HttpResponseObject[]]> | HttpResponseObject | ZodUndefined;
};
export type HttpObject = ZodObject<Http, any, objectUtil.Flatten<objectUtil.ObjectType<Http>>>;
...
function createOperationObject(shape: Http): OperationObject {
    return {
        summary: ("value" in shape.summary._def)
            ...
            : undefined,
        operationId: ("value" in shape.name._def) ?shape.name._def.value : undefined,
...
function createParameterObject(shape: Http) {
    const res = [
        ...("shape" in shape.query._def) ? createQueryParameterObject(shape.query._def.shape()) : [],
        ...("items" in shape.path._def) ? shape.path._def.items.filter(it => 'state' in it).map(createPathParameterObject) : []
    ]
    return res.length > 0 ? res : undefined
}

function createPathParameterObject(it: ZodLiteral<any> | ZodString | Parameter): ParameterObject {
    return {
...
function createQueryParameterObject(it: Record<PropertyKey, Parameter>): ParameterObject[] {
...
function createResponsesObject(responses: ZodUnion<[HttpResponseObject, HttpResponseObject, ...HttpResponseObject[]]> | HttpResponseObject | ZodUndefined): Record<number, ResponseObject> {
...
function createResponseObject(response: HttpResponse): Record<number | string, ResponseObject> {
    const name = ("value" in response.status._def) ? response.status._def?.value : 'default'
    return {
        [name]: {
            description: response.description._def.value,
            headers: ("shape" in response.headers._def)
                ? createHeadersObject(response.headers._def.shape()) : undefined,
...
function createHeadersObject(it: Record<PropertyKey, Parameter>): HeadersObject {
