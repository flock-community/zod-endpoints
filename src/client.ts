...
type ClientMatch<T extends z.HttpSchema, R extends ClientRequest<T>> = Extract<ClientRequestResponses<T>, ClientMapper<T, R>>['responses']

export type Client<T extends z.HttpSchema> = <R extends ClientRequest<T>>(req: R) => Promise<ClientResponseAttributes extends keyof ClientMatch<T, R> ? PickUnion<ClientMatch<T, R>, ClientResponseAttributes> : undefined>;
